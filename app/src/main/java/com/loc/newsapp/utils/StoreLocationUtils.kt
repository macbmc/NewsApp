...
class StoreLocationUtils(
    private val appContext: Context, private val sharedPreference: SharedPreferences
) {
                ...
                if (it.isSuccessful) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                        ...
                        launch {
                            send(
                                sharedPreference.edit().putString(
                                        "DEVICE_LOCATION_COUNTRY", geoLocation!![0].countryName
                                    ).putString(
                                        "DEVICE_LOCATION_COUNTRY_CODE", geoLocation[0].countryCode
                                    ).commit()
                            )
                        }
                    }
